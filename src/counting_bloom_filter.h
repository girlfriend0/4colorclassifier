...
public:
    ...
    vector<uint32_t> query_multiway(uint64_t key)  // find key in which class
    {
...
template<int num_bits, int k, int class_num = 2, int count_bits=4>
class CountingBloomFilterGroup
{
    CountingBloomFilter<num_bits, k, class_num, count_bits> cbfs[class_num];

public:
    CountingBloomFilterGroup()
    {
        for (int i = 0; i < class_num; i++)
            cbfs[i].class_idx = i;
    }

    void insert(uint64_t item, uint32_t class_id)
    {
        for (int i = 0; i < class_num; i++)
            cbfs[i].insert(item, class_id);
    }

    void remove(uint64_t item, uint32_t class_id)
    {
        cbfs[class_id].remove(item);
    }

    vector<uint32_t> query(uint64_t item)
    {
        return cbfs[item % class_num].query_multiway(item);
    }
};
